...
\titlespacing{\subsection}{0pt}{25pt}{12pt}[0pt]
...
\begin{enumerate}[(i)]
	...
	\item Über die rekursive Darstellung der Legendre-Polynome lassen sich lokale Beziehungen zwischen Nullstellen zweier Polynome $ L_n $ und $ L_{n+1} $ herleiten. Mittels bekannter Nullstellen des $ n $-ten Polynomes und eines Bisektions- bzw. Newton-Verfahrens wurden die Nullstellen von $ L_{n+1} $ ermittelt.
\end{enumerate}
...
\subsection{Direkte Knotenberechnung} 
Wegen der Symmetrie um den Ursprung genügt es die nichtnegativen Nullstellen zu be\-trachten. Für ungerades $ n $ ist $ 0 $ stets ein Quadraturknoten, also eine Nullstelle von $ L_n $.
\newline
...
\begin{enumerate}[(1)]
	\item \textbf{while} (($ |f(x_1)|>TOL $ or $ |x_1-x_2|>TOL $) and $ f''(x_1)f''(x_2)>0 $) \textbf{do:}
	\item ein Durchlauf des Bisektions-Verfahrens mit $ x_1 $ und $ x_2 $ (Skriptum: Algorithmus 7.1);
	\item \textbf{end do;}
	\item \textbf{while} ($ |f(x_t)|<TOL $) \textbf{do:}
	\item ein Durchlauf der Newton-Iteration mit Startwert $ x_1 $ (Skriptum: Algorithmus 7.12, allerdings mit anderem Abbruchkriterium);
	\item \textbf{end do;}
\end{enumerate}
...
\subsection{Explizite Darstellung der Gewichte} 
