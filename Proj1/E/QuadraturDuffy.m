function [ retval ] = QuadraturDuffy(n ,Interval1, Interval2, varargin )
%QUADRATUR führt Quadratur mit n+1 x n+1 Stützstellen auf dem linken unteren Dreieck
%in durch[a,b]x[c,d] durch.
...
for i = m+1:nargin
    retval(i-m) = 0;
    for k = 1:n+1
        duffyX = 1-(samplePtsx(k)-a)/(b-a);
        retval(i-m) = retval(i-m) + sum(double(varargin{i-m}(samplePtsx(k), samplePtsy*duffyX).*weightsy))*weightsx(k)*duffyX;
    end
end
