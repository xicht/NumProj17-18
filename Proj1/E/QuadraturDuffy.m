function [ retval ] = Quadratur2Duffy(n ,Interval1, Interval2, varargin )
...
for i = m+1:nargin
    ...
    retval(i-m) = 0;
    for k = 1:n+1
        duffyY = 1-samplePtsy(k)/(d-c);
        retval(i-m) = retval(i-m) + sum(double(varargin{i-m}(samplePtsx*duffyY, samplePtsy(k)) .* Weightsx * Weightsy(k)*duffyY));
    end
end
